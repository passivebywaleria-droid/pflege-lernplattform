...
export function StepCalculation({
  ...
  const isCorrect = !isNaN(numericValue) && Math.abs(numericValue - correctValue) <= tolerance;
  const isClose = !isCorrect && !isNaN(numericValue) && Math.abs(numericValue - correctValue) <= tolerance * 3;
          ...
          >
            <p className="font-semibold">
              {isCorrect
                ...
                : isClose
                  ? `Fast! Du hast ${input} ${unit} berechnet, richtig wäre ${correctValue} ${unit}.`
                  : `Leider nicht. Richtig wäre ${correctValue} ${unit}.`}
            </p>
          </motion.div>
