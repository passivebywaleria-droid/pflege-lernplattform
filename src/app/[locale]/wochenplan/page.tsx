...
const TYP_FARBEN: Record<string, { bg: string; text: string; badge: string }> = {
  neu: { bg: "bg-[#5A7D60]/5", text: "text-[#5A7D60]", badge: "bg-[#5A7D60]/10 text-[#5A7D60]" },
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border-2 transition-all ${
        istHeute
          ? "border-[#5A7D60] shadow-lg shadow-[#5A7D60]/10"
          : "border-transparent"
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className={`text-sm font-bold ${istHeute ? "text-[#5A7D60]" : "text-[var(--lern-text-primary)]"}`}>
            {TAG_NAMEN[tag.tag]}
          </span>
          {istHeute && (
            <span className="rounded-full bg-[#5A7D60] px-2 py-0.5 text-xs font-semibold text-white">
              {t("today")}
            </span>
          )}
        </div>
      ...
      {istHeute && (
        <Link
          ...
          className="block w-full rounded-xl bg-[#5A7D60] px-4 py-3 text-center text-sm font-semibold text-white transition-all active:scale-[0.98] hover:bg-[#4C6A52]"
        >
          {t("startNow")}
        </Link>
      )}
        ...
        <div className="mx-auto max-w-2xl px-4 py-3">
          <div className="flex items-center justify-between">
            <Link
              ...
              className="text-sm font-medium text-[#5A7D60] active:opacity-60"
            >
              Zurück
            </Link>
      ...
      <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#5A7D60]/10">
              <span className="text-lg font-bold text-[#5A7D60]">
                {gesamtFortschritt}%
              </span>
            </div>
          </div>
