...
- **1.2 ✅ `dozentin-recherche`-Agent umgebaut (2026-06-07):** Neue Pflicht-Sektion QUELLENBINDUNG: pro Fakt passende Passage über ALLE Grounding-Quellen (`dnqp-standards-index/`, `*-volltext/`) suchen (flexibel/Synonyme: Post-Fall-Syndrom=Sturzphobie), kurze Verbatim-Phrase wählen, mit `zitat-verifizierer` prüfen, als `**Beleg:**`-Block eintragen. Ungedeckte Fakten → „⚠️ Quelle beschaffen", NIE erfinden. Abschluss-Pflicht: `--check-file` muss „✅ Alle Belege verifiziert". Format + Beispiel + Workflow + Checkliste (K.O.) angepasst.
...
- **1.4 ✅ `scripts/faktentreue-check.ts` gebaut (2026-06-07):** prüft, ob jedes benannte klinische INSTRUMENT/STANDARD/SKALA im Content in der Faktenbasis (Kernfakten + Glossar) gedeckt ist. Deterministisch, rauscharm (Patienten-Vitalwerte ausgenommen — authored Fall-Daten, keine Autoritäts-Claims). Erster Lauf CE-02: 21 Instrumente, 18 gedeckt, **3 ungedeckt (ABCDE, STRATIFY, ZOPA)** → echte Review-Kandidaten (Coverage-Lücke oder erfunden). Wird scharfes Gate, sobald Kernfakten vollständig (Phase 3/4). Erweiterung Schwellen-Claims: später (verrauscht wegen Fall-Daten).
