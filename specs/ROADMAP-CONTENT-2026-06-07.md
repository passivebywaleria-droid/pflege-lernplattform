...
- **1.3 Neuer `scripts/zitat-verifizierer.ts`:** prüft jedes `woertlichesZitat` per String-Match gegen die zitierte Quelldatei. Kein Treffer → Fail. Maschinell, billig, deterministisch (durch Rev.-2-Beleg-Format möglich). Plus adversarialer KI-Check „passt Paraphrase zum Zitat?".
